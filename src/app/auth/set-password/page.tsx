...
function SetPasswordContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) { setSuccess(true); setTimeout(() => router.push("/auth/login?welcome=1"), 2500); }
      else { setError(data.error || "Error al establecer la contrasena"); }
    } catch { setError("Error de conexion"); }
    finally { setLoading(false); }
  };

  if (tokenValid === null) return <div className={styles.container}><div className={styles.card}><p style={{textAlign:"center",color:"#666"}}>Verificando enlace...</p></div></div>;
  if (tokenValid === false) return (
    <div className={styles.container}><div className={styles.card}>
      <div className={styles.header}><h1 className={styles.title} style={{color:"#ef4444"}}>Enlace invalido</h1></div>
      <p style={{textAlign:"center",color:"#666",marginBottom:"1.5rem"}}>El enlace expiro o ya fue utilizado. Contacta a tu administrador.</p>
      <Link href="/auth/login" style={{display:"block",textAlign:"center",color:"#16a34a"}}>Ir al login</Link>
    </div></div>
  );
  if (success) return (
    <div className={styles.container}><div className={styles.card}>
      <div className={styles.header}><h1 className={styles.title} style={{color:"#16a34a"}}>Contrasena establecida!</h1></div>
    ...
    <div className={styles.container}>
      <div className={styles.card}>
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <button type="submit" className={styles.submitBtn} disabled={loading}>{loading ? "Guardando..." : "Establecer contrasena"}</button>
        </form>
      </div>
    </div>
  );
